...
- **Admin → FAZ Targets**: CRUD for the FortiAnalyzer appliances the
  ...
  the next poll cycle without an app restart, and a per-target **Threat Polling** checkbox (default on) controlling whether it's included in the threat-activity collector below.
